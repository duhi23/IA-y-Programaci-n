...
:::{.callout-morado .column-margin}
💡 **Tip:** En R se usa `<-` para asignar variables, mientras que en Python se usa `=`.
:::
...
:::{.callout-morado .column-margin}
💡 **Tip:** En Python por medio de la función `id()` se puede obtener la dirección de memoria donde se almacena el objeto referenciado.

a = [1, 2, 3]

b = a  # Misma dirección de memoria

b.append(4)

print(a)  # Cambio de dirección en la memoria

En R no existe una función base nativa directa que devuelva la dirección de memoria, sin embargo, por medio del paquete `lobstr` se puede emplear la función `obj_addr()`.
:::
...
En Python existe una convención ampliamente aceptada por la comunidad para **importar las librerías principales** de ciencia de datos y desarrollo con alias estandarizados de dos o tres letras.

El objetivo de este acuerdo es mantener el código conciso, legible y uniforme entre proyectos y equipos de trabajo.

| Librería | Alias | Código de Importación | Área / Uso |
| :--- | :--- | :--- | :--- |
| **NumPy** | `np` | `import numpy as np` | Cómputo numérico y arreglos |
| **Pandas** | `pd` | `import pandas as pd` | Manipulación y análisis de datos |
| **Matplotlib (Pyplot)** | `plt` | `import matplotlib.pyplot as plt` | Visualización y gráficos base |
| **Seaborn** | `sns` | `import seaborn as sns` | Visualización estadística avanzada |
| **TensorFlow** | `tf` | `import tensorflow as tf` | Deep learning y redes neuronales |
| **PyTorch (Functional)** | `F` | `import torch.nn.functional as F` | Redes neuronales (submódulos) |
| **Statsmodels (API)** | `sm` | `import statsmodels.api as sm` | Modelos estadísticos y econometría |
| **Statsmodels (Formula)** | `smf` | `import statsmodels.formula.api as smf` | Modelos estilo R (`y ~ x`) |
| **Polars** | `pl` | `import polars as pl` | DataFrames de alto rendimiento |
| **OpenCV** | `cv2` | `import cv2` | Visión por computadora |
...
**Ejercicio 3:** Calcular el porcentaje de descuento asignado a una compra según el tipo de cliente (`VIP o Estándar`), el monto total y si el cliente cuenta con un cupón promocional activo.
  ...
  - **Descuento del 20%**: Si el cliente es VIP y el monto supera 150 dólares, o si cuenta con cupón promocional (`TRUE`).
  - **Descuento del 10%**: Si el cliente es VIP pero el monto es menor o igual a 150 dólares, o si es cliente Estándar con un monto superior a 200 dólares.
  - **Sin descuento (0%)**: En cualquier otro caso.
...
# Evaluación de condicionales
if (____ and ____) or ____:
    descuento = ____
elif ____ or (____ and ____):
    descuento = ____
else:
    descuento = ____
...
:::{.callout-morado .column-margin}
💡 **Tip:** Usa `for` cuando sabes exactamente cuántas veces iterar; usa `while` cuando dependes de una condición que cambia sobre la marcha.
:::
...
::: {.ejercicio-resuelto}
**Ejemplo 4:** Recorra un vector/lista de calificaciones y contabilice cuántas
están aprobadas (mayor o igual a 7), usando un bucle `for`.
...
**Ejercicio 4**: La *serie de Leibniz* permite calcular una aproximación del valor de $\pi$ a través de una suma infinita de términos alternados:
$$\pi = 4\cdot \sum_{k=0}^{\infty} \frac{(-1)^k}{2k+1}$$
Escriba un código que permita calcular la aproximación de $\pi$ sumando los primeros $N$ términos de la serie.
...
**Ejercicio 5**: Escriba un código que reciba un número entero positivo $n$ y verifique experimentalmente el Teorema de Nicómaco que establece la siguiente identidad matemática:
...
**Ejemplo 6:** Escribir una función que reciba dos enteros $n$ y $k$, con $n\geq k$ y calcule la cantidad de subconjuntos $C_{n}^{k}$ distintos de $k$ elementos que se pueden formar a partir de un conjunto de $n$ elementos, es decir:
$$C_{n}^{k} = \frac{n\cdot(n-1)\cdot(n-2)\ldots (n-k+1)}{1\cdot 2\cdot 3\ldots k}$$
...
**Ejemplo 7:** Utilizando la función `combinacion(n, k)` definida previamente, escriba una nueva función que reciba un entero no negativo $n$ y calcule el valor de $2^n$ aprovechando la propiedad de la suma de los coeficientes binomiales:
...
**Ejercicio 6**: Crear una función que reciba un número entero $n > 0$ y devuelva
`TRUE`/`True` si es primo, `FALSE`/`False` en caso contrario. 
...
def contar_primos(a: int, b: int) -> int:
...
leen desde archivos externos (texto, CSV, Excel) y, tras procesarlos,
...
|--------------|--------------------------------------|----------------------------------|
| Texto plano  | `read.table()` / `write.table()`      | `open()` + `.read()`/`.write()` |
| CSV          | `readr::read_csv()` / `readr::write_csv()`        | `pd.read_csv()` / `.to_csv()`   |
| Excel        | `readxl::read_excel()` / `openxlsx::write.xlsx()` | `pd.read_excel()` / `.to_excel()` |
:::

![Archivos Externos](imgs/json.png){.column-margin width=250px}
...
write.xlsx(datos, "data/Seguros_Extra.xlsx")
```
...
datos.to_excel("data/Seguros_Extra.xlsx", index=False)
```
:::

:::{.callout-morado .column-margin}
💡 **Tip:** Usa siempre **rutas relativas** dentro del proyecto (por
ejemplo `"data/archivo.csv"`) en lugar de rutas absolutas de tu
computador, para que los archivos funcionen igual en cualquier máquina y al
subirlo a GitHub.
:::
...
**Ejemplo 8:** Crear un dataframe con datos de 5 estudiantes, guárdalo como
CSV, y vuelva a leerlo para confirmar que se guardó correctamente.
